...
#define PARSE(doc, input) \
	...
	LOCAL_YAML::Parser parser(stream);\
	LOCAL_YAML::Node doc;\
...
namespace Test {
	...
	namespace Spec {
		...
		void operator >> (const LOCAL_YAML::Node& node, Pair& p) {
			node[0] >> p.first;
			node[1] >> p.second;
		}
		...
		TEST InvalidEscapedCharacters()
		{
			...
			try {
				...
				LOCAL_YAML::Node doc;
				parser.GetNextDocument(doc);
			} catch(const LOCAL_YAML::ParserException& e) {
				YAML_ASSERT(e.msg == LOCAL_YAML::ErrorMsg::INVALID_ESCAPE + "c");
				return true;
			}
		...
		void operator >> (const LOCAL_YAML::Node& node, StringMap& m) {
			m._.clear();
			for(LOCAL_YAML::Iterator it=node.begin();it!=node.end();++it) {
		...
		TEST CompletelyEmptyNodes()
		{
			...
			YAML_ASSERT(doc[LOCAL_YAML::Null] == "bar");
			return true;
		}
